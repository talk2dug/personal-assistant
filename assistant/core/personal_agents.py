...
from . import agent_notes, db, personal_db
...
def run_personal_research_queue(db_path: str, llm, owner_user_id: int, limit: int = 2,
                                obsidian=None) -> dict:
    ...
    `obsidian` (an ObsidianClient) also files each completed brief into the vault's agent
    folder -- same shape and same best-effort contract as agents.run_research_queue. The
    findings land in personal_research either way; the note is the copy he can actually
    ...
    for item in queued:
        ...
        try:
            ...
            # After the database write, never before -- see agents.run_research_queue.
            agent_notes.write_research_brief(
                obsidian, item["topic"], findings or "",
                question=item.get("question"), source="personal")
